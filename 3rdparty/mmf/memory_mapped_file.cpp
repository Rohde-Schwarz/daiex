...
namespace memory_mapped_file
{
        ...
        size_t offset, size_t requested_size)
    {
        ...
        file_mapping_handle_ = ::CreateFileMapping(
            file_handle_, 0, PAGE_READONLY, (offset + mapping_size) / 0xFFFFFFFF,
        ...
        char* real_data = static_cast<char*>(::MapViewOfFile(
            file_mapping_handle_, FILE_MAP_READ, real_offset / 0xFFFFFFFF,
...
#if defined(_WIN32)
    writable_mmf::writable_mmf(wchar_t const* pathname,
...
#if defined(_WIN32)
    void writable_mmf::open(wchar_t const* pathname,
        memory_mapped_file::mmf_exists_mode exists_mode,
        memory_mapped_file::mmf_doesnt_exist_mode doesnt_exist_mode)
#else
...
#endif
    {
        ...
        file_handle_ = ::CreateFileW(pathname, GENERIC_READ | GENERIC_WRITE,
            FILE_SHARE_READ | FILE_SHARE_WRITE, 0,
    ...
    #if defined(_WIN32)
        file_mapping_handle_ = ::CreateFileMapping(
            file_handle_, 0, PAGE_READWRITE, (offset + mapping_size) / 0xFFFFFFFF,
            (offset + mapping_size) & 0xFFFFFFFF, 0);
        if (file_mapping_handle_ == INVALID_HANDLE_VALUE) return;
        char* real_data = static_cast<char*>(::MapViewOfFile(
            file_mapping_handle_, FILE_MAP_WRITE, real_offset / 0xFFFFFFFF,
            real_offset & 0xFFFFFFFF, offset - real_offset + mapping_size));
        if (! real_data) return;    
    #else
